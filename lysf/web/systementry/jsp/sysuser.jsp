...
<body>
    ...
    <!-- 用户详情弹出框内容 -->
    <div class="modal" id="userxiangqing" role="dialog" tabindex="-1"
         aria-labelledby="demo-default-modal" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-header" style="border-bottom: 1px solid #e5e5e5;">
                    ...
                    <h4 class="modal-title">用户详情界面</h4>
                </div>

                <div id="userthead"></div>
  ...
  $('#demo-dt-basic').on('draw.dt',function() {
    ...
    $('#demo-dt-basic tbody').on('click','.xiugai', function() {
                    ...
                    var sno=$('#demo-dt-basic').DataTable().row(rowIndex).data().sno;
                        ...
                        if ($("#form1").data('bootstrapValidator').isValid()) {
                            $.ajax({
                                ...
                                success: function (data) {
                                    ...
                                    $("#xiugai").modal('hide');
                                    console.log($(this))
                                    this1.children(":eq(1)").text(data.sno)
                                    this1.children(":eq(2)").text(data.username)
                                    this1.children(":eq(3)").text(data.password)
                                    this1.children(":eq(4)").text(data.email)
                                    this1.children(":eq(5)").text(data.phone)
                                    this1.children(":eq(6)").text(data.rname)
									//把修改后的值放入到datatable中
                                    $('#demo-dt-basic').DataTable().row(rowIndex).data().sno = data.sno;
   	...
   	//用户详情
     $('#demo-dt-basic tbody').on('click','.chakan', function(){
         $("#userthead").empty();
         var thisis = $(this).parent().parent();
         thisis.siblings().removeClass('tog')
         thisis.toggleClass('tog')
                 ...
                 success:function(data) {
                     if(data==null&&data==""){
                         ...
                         $("#userthead").append(
                             ...
                             '<div>用户编号：'+data.uid+'</div>'+
                             '<div>用户性别：'+data.sex+'</div>'+
                             '<div>用户年龄：'+data.age+'</div>'+
                             '<div>用户学院：'+data.college+'</div>'+
                             '<div>用户年级：'+data.grade+'</div>'+
                             '<div>用户班级：'+data.uclass+'</div>'+
