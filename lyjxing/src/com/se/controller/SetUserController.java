...
public class SetUserController {
	...
	public String setUser(String username,
			String password,String signed,String registerAddress,String email,HttpServletRequest request){
		//获取用户对象
		SysUser hu = (SysUser) request.getSession().getAttribute("USERINFO");
		//更新用户数据信息
		hu.setEmail(email);
		hu.setUsername(username.trim());
		hu.setSigned(signed.trim());
		hu.setPassword(password.trim());
		hu.setRegisterAddress(registerAddress.trim());
